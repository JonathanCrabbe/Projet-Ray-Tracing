...
for x = 0:0.5:7
    for y = 0:0.5:10.5 
        ...
        G = stationBase.getGain(pi/2); %Gain dans la direction consideree
        E = directRay.getE(G); %Calcul du champ arrivant au recepteur
        he = recepteur.getHauteur(pi/2); %Hauteur l'equivalente de l'antenne
        ...
        for i = 1:(numel(wallList)) %Pour chaque mur:
          ...
          he = recepteur.getHauteur(pi/2); %Hauteur equivalente de l'antenne
